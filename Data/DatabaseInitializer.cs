...
public sealed class DatabaseInitializer(NpgsqlDataSource dataSource, IConfiguration configuration, PasswordService password)
{
    public async Task InitializeAsync()
    {
        ...
        CREATE TABLE IF NOT EXISTS menu_items (id serial PRIMARY KEY,category varchar(60) NOT NULL,name varchar(100) NOT NULL,description text NOT NULL,price numeric(10,2) NOT NULL,sort_order int NOT NULL DEFAULT 0,is_available boolean NOT NULL DEFAULT true);
        ...
        await db.ExecuteAsync("ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS sort_order int NOT NULL DEFAULT 0; UPDATE menu_items m SET sort_order = ranked.sort_order FROM (SELECT id, row_number() OVER (ORDER BY category,name) * 10 AS sort_order FROM menu_items) ranked WHERE m.id=ranked.id AND m.sort_order=0;");
        ...
        if (await db.ExecuteScalarAsync<int>("SELECT count(*) FROM menu_items") == 0)
            await db.ExecuteAsync("INSERT INTO menu_items(category,name,description,price,sort_order) VALUES ('Breakfast','Lakeside Sunrise Skillet','Eggs, breakfast potatoes, cheddar, peppers and toast.',13.50,10),('Breakfast','Blueberry Pancake Stack','Three buttermilk pancakes with local blueberries and maple syrup.',11.00,20),('Lunch','Dockside Burger','Griddled beef, cheddar, crispy onions, pickles and fries.',15.00,30),('Lunch','Cedar Chicken Wrap','Grilled chicken, greens, tomato, ranch and a warm tortilla.',13.50,40),('Dinner','Friday Lake Perch Basket','Crisp lake perch, slaw, fries and tartar sauce.',18.00,50),('Dinner','Campfire Flatbread','Mozzarella, roasted vegetables, pesto and a bright side salad.',16.00,60),('Kids','Little Loon Grilled Cheese','Cheddar toastie with apple slices and a cookie.',7.50,70),('Drinks','Wild Berry Lemonade','House lemonade, wild berry syrup and fresh mint.',4.75,80),('Drinks','Campground Cocoa','Rich hot cocoa topped with toasted marshmallows.',4.25,90)");
